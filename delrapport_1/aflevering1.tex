...
\item All teachers have their OpenPGP-keys on the service and sign all feedback.
\end{itemize}
...
% skriv om!
\item The upload and initial authentication happens through the KU email system. (gælder også for replacement)
\item Lookups in this must be possible for university staff involved in teaching.
%skal skrives om!
\item must not be able for outsiders
\item If a user loses his private key, a method for replacing the public key must exits.
\item A guide for creating key pairs needs to be on the front-end (possibly as a shell script.) These guides should include how to extract a RSA-key (SSH-key) from the created OpenPGP-key.
\end{itemize}
...
%der kan godt være del-produkter.
\item Beta ready at start/mid may.
\end{itemize}
...
\begin{itemize}
\item Set up VPS to start Go-server and make database and create database schema upon startup.
...
\item Determine exact method for teacher's to fetch data from DB
\item Front-end view containing a form for first-time upload of OpenPGP key.
\item Front-end view which enables updating/replacement of key.
...
\item Sven Frenzel: Linux server administration, basic network administration.
...
We have chosen to divide the project schedule into phases. But the whole list should be viewed as a sort of iterative process and something that will overlap a lot. An overview of the schedule is presented in the following list. Some of these tasks could also be given a deadline under Section \ref{sec:deadlines}.
\begin{itemize}
\item Defining the project -- the introductory phase where we iteratively align our expectations for the project with the customers expectations.
\item Learning Go -- this can be done through some very basic Go tutorials but will probably mainly be achieved by reading up on how a Go server is built and connects to a server.
\item Obtaining additional required skill set. Some front-end development in HTML5 or Javascript. 
\item Setting up server with required programs, including e.g. Go compiler and SQL server.
\item Programming the server and its SQL interaction in Go.
\item Programming front-end
\item Setting up email server to talk to Go on our VPS
\item Presenting result to customer for feedback
\item Testing the programmed logic.
\end{itemize}
\subsubsection{Budget and Resources}
\begin{itemize}
\item VPS or dedicated server (DIKU supplies)
\item access to a mail server (Oleks researches)
\item Domain or subdomain to host the PGP registration website
\item SSL certificate to defeat man-in-the-middle attack where an attacker sends a fake public PGP key to the server.
\item time resources: As of yet unknown but we expect that large parts of the programming can be solved at intencive work weekends
\end{itemize}
\subsubsection{Description of Risks}
There is a risk that learning Go will be to complex a task for us to master in time. To mitigate this risk, a deadline for some basic server functionality, using Go, should be made. We set a deadline for May 10$^{th}$ for getting the VPS to connect to the database through Go.
